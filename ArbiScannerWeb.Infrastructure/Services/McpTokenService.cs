...
public class McpTokenService(
    ...
    ILogger<McpTokenService> logger) : IMcpTokenService
{
    private readonly HttpClient _httpClient = httpClientFactory.CreateClient("KeycloakMcpExchange");
    ...
    public async Task<Result<string>> GenerateAccessTokenAsync(CancellationToken cancellationToken)
    {
        ...
        var clientId = configuration["Keycloak:McpExchange:ClientId"];
        var clientSecret = configuration["Keycloak:McpExchange:ClientSecret"];
        ...
        try
        {
            ...
            {
                logger.LogError("Keycloak token exchange response did not contain an access_token: {Payload}", payload.GetRawText());
                return Result.Fail(TypedErrors.InternalError("Could not generate an MCP token. Please try again later."));
            }
        ...
        {
            logger.LogError(ex, "Error exchanging token with Keycloak for an MCP token");
            return Result.Fail(TypedErrors.InternalError("Could not generate an MCP token. Please try again later."));
        }
    }
}
